...
def conv2d(x, fmaps, kernel, gain=np.sqrt(2), use_wscale=False, strides=[1,1,1,1]):
    assert kernel >= 1 and kernel % 2 == 1
    w = get_weight([kernel, kernel, x.shape[1].value, fmaps], gain=gain, use_wscale=use_wscale)
    w = tf.cast(w, x.dtype)
    return tf.nn.conv2d(x, w, strides=strides, padding='SAME', data_format='NCHW')
...
def D_paper(
    ...
    latents_in,                            # Input: Latents [minibatch, latent_size].
...
#----------------------------------------------------------------------------
def Encoder(
    images_in,                         # First input: Latent vectors [minibatch, latent_size].
    num_channels = 1,                  # Number of input color channels. Overridden based on dataset.
    resolution  = 32,                  # Input resolution. Overridden based on dataset.
    lvls = 4,
    gf_dim = 64,
    ...
    **kwargs):                          # Ignore unrecognized keyword args.
    ...
    with tf.variable_scope("Encoder"):
        e = images_in
        for l in range(lvls):
            c = int(gf_dim * 2**(min(l, 3)))
            with tf.variable_scope( "Conv" + str(l) ):
                e = act(apply_bias(conv2d(e, fmaps=c, kernel=3, strides=[1,1,2,2], use_wscale=use_wscale)))
        ...
        h1 = apply_bias( dense(e, fmaps=z_len, use_wscale=use_wscale) )
    return h1
