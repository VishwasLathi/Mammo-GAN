...
def save_all_res( max_res, Gs, res_subdir, num_pngs, grid_size=[1,1], random_seed=1000, minibatch_size=8  ):
    ...
    while ( shrink <= 5 ):
        ...
        for png_idx in range(num_pngs):
            ...
            images = Gs.run(latents, labels, minibatch_size=minibatch_size, num_gpus=1, out_mul=127.5, out_add=127.5, out_shrink=int( pow(2,shrink-1) ) , out_dtype=np.uint8)
            save_image_grid(images, os.path.join(res_dir, '%06d.png' % (png_idx)), [0,255], grid_size)
        shrink += 1
